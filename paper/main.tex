...
% Double-blind: keep PDF metadata free of identifying information.
\hypersetup{pdfauthor={Anonymous}, pdftitle={}, pdfsubject={}, pdfkeywords={}}
...
\title{Meta-Learning for Few-Shot Personalization of\\Error-Related Potential Brain-Computer Interfaces}
...
% ===================================================================
\begin{abstract}
When a brain-computer interface (BCI) errs, the user's electroencephalogram (EEG)
carries an error-related potential (ErrP) that the system can use to self-correct.
Because ErrP morphology varies across people, each new user must sit through a long
calibration session. We ask whether meta-learning can shorten it: instead of one
decoder per user, a meta-model learns an initialization that a new user adapts from
a few class-balanced trials. Under one leave-one-subject-out protocol, run
separately on two structurally different ErrP datasets (not trained across them), we
benchmark ten methods (three gradient-based and two metric-based meta-learners, a
FiLM subject-conditioned encoder, and four classical baselines including a
source-pretrained EEGNet) plus two transfer controls that share the source data. At
a five-trial budget, the gradient meta-learners, the FiLM encoder, and EEGNet all
significantly beat a from-scratch supervised baseline on both datasets (subject-level
Wilcoxon, $N{=}16$, BH-FDR $p<0.05$). The finding \emph{replicates} on the second
paradigm, where the meta-learners, the FiLM encoder, and Prototypical networks also
beat EEGNet at $K{=}5$, though this edge closes by $K{=}10$. The meta-learners
further surpass transfer baselines matched on source data and class weighting; since
test-time adaptation still differs, we credit the meta-learning approach rather than
data access alone. Ablations show class-weighted episodic loss is the dominant design
choice, while FiLM and feature dimensionality add little. We frame the result as
improved low-data personalization, not a deployment-ready decoder.
\end{abstract}
